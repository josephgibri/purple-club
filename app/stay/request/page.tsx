...
/**
 * Concierge request form — paste a hotel link and submit a private-rate
 * request. Wallet connect / verify is owned by the global Purple Club header
 * (TopNav); this page reads the resulting server session to decide whether
 * the member can submit, and offers an inline sign-in trigger as a fallback.
 */
export default function HotelRequestPage() {
  ...
  // Read the server session minted by the global Purple Club auth. Re-fetch on
  // connect / verify; one delayed retry absorbs the cookie-mint race.
  useEffect(() => {
    let cancelled = false;
    ...
    void loadSession().then((settled) => {
      if (!settled && !cancelled && isVerified) {
        retry = setTimeout(() => void loadSession(), 1200);
      }
    });
    return () => {
      cancelled = true;
  ...
  const canSubmit = session.authenticated && session.pbtcEligible;
    ...
    <main className="relative flex min-h-screen flex-col">
      ...
      <section className="relative z-10 mx-auto w-full max-w-3xl px-6 py-12">
        ...
        <p className="mt-3 max-w-xl text-sm text-white/65">
          Paste your hotel link and our concierge will negotiate wholesale pricing.
        </p>
